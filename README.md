...
| CA212 | Scoped to **Office 365** (browser, BYOD Windows). Targets unmanaged Windows devices using the device filter `mdmAppId -ne Intune -and isCompliant -ne True`. App protection policy (`compliantApplication`) enforced via Intune MAM. Defender for Cloud Apps session control blocks downloads. |
| CA213–CA214 | Scoped to **All cloud apps** on Android and iOS BYOD. Device filter uses `mdmAppId -ne Intune -or deviceOwnership -eq Personal` (include mode) to cover both unmanaged and personally-owned enrolled devices. `excludeApplications` lists apps incompatible with App Protection Policy (Intune enrollment portal, Microsoft authenticator registration, and Azure AD/AAD Graph) to prevent MAM from blocking registration flows. Review and extend `excludeApplications` for any additional non-MAM-capable apps before enabling. CA215 remains scoped to Office 365 — Application Enforced Restrictions is a SharePoint/Exchange server-side feature only. |
| CA213–CA214 (passkey note) | **Baseline Note:** Microsoft documents a known limitation with Microsoft Authenticator passkey registration on iOS and Android: users can be blocked when a Conditional Access policy targets All resources and requires App protection policy. For passkey registration compatibility, Microsoft's documented workaround is to avoid targeting All resources for these policies and instead scope the policy to selected supported applications, such as Office 365 and other validated apps. Current baseline keeps All resources with explicit exclusions for required registration services. This provides broader BYOD protection, but admins should understand that this is an exception-based design and may require future adjustment if Microsoft changes the Authenticator registration flow. Reference: [Microsoft Authenticator passkey Conditional Access limitation](https://learn.microsoft.com/en-us/entra/identity/authentication/how-to-support-authenticator-passkey) |
